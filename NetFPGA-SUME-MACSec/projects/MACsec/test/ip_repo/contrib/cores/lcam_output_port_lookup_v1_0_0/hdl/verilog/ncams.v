...
//
//  Module:
//        This file has CAM instances for large CAM.
//
//  Description:
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_0 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 0) ? we : 0;
		addr_wr_0 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 0) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_0 <= din;
		cmp_din_0 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_1 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 1) ? we : 0;
		addr_wr_1 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 1) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_1 <= din;
		cmp_din_1 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_2 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 2) ? we : 0;
		addr_wr_2 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 2) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_2 <= din;
		cmp_din_2 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_3 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 3) ? we : 0;
		addr_wr_3 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 3) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_3 <= din;
		cmp_din_3 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_4 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 4) ? we : 0;
		addr_wr_4 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 4) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_4 <= din;
		cmp_din_4 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_5 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 5) ? we : 0;
		addr_wr_5 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 5) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_5 <= din;
		cmp_din_5 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_6 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 6) ? we : 0;
		addr_wr_6 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 6) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_6 <= din;
		cmp_din_6 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_7 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 7) ? we : 0;
		addr_wr_7 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 7) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_7 <= din;
		cmp_din_7 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_8 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 8) ? we : 0;
		addr_wr_8 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 8) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_8 <= din;
		cmp_din_8 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_9 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 9) ? we : 0;
		addr_wr_9 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 9) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_9 <= din;
		cmp_din_9 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_10 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 10) ? we : 0;
		addr_wr_10 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 10) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_10 <= din;
		cmp_din_10 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_11 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 11) ? we : 0;
		addr_wr_11 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 11) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_11 <= din;
		cmp_din_11 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_12 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 12) ? we : 0;
		addr_wr_12 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 12) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_12 <= din;
		cmp_din_12 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_13 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 13) ? we : 0;
		addr_wr_13 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 13) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_13 <= din;
		cmp_din_13 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_14 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 14) ? we : 0;
		addr_wr_14 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 14) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_14 <= din;
		cmp_din_14 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_15 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 15) ? we : 0;
		addr_wr_15 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 15) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_15 <= din;
		cmp_din_15 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_16 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 16) ? we : 0;
		addr_wr_16 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 16) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_16 <= din;
		cmp_din_16 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_17 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 17) ? we : 0;
		addr_wr_17 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 17) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_17 <= din;
		cmp_din_17 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_18 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 18) ? we : 0;
		addr_wr_18 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 18) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_18 <= din;
		cmp_din_18 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_19 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 19) ? we : 0;
		addr_wr_19 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 19) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_19 <= din;
		cmp_din_19 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_20 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 20) ? we : 0;
		addr_wr_20 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 20) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_20 <= din;
		cmp_din_20 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_21 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 21) ? we : 0;
		addr_wr_21 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 21) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_21 <= din;
		cmp_din_21 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_22 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 22) ? we : 0;
		addr_wr_22 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 22) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_22 <= din;
		cmp_din_22 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_23 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 23) ? we : 0;
		addr_wr_23 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 23) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_23 <= din;
		cmp_din_23 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_24 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 24) ? we : 0;
		addr_wr_24 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 24) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_24 <= din;
		cmp_din_24 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_25 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 25) ? we : 0;
		addr_wr_25 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 25) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_25 <= din;
		cmp_din_25 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_26 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 26) ? we : 0;
		addr_wr_26 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 26) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_26 <= din;
		cmp_din_26 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_27 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 27) ? we : 0;
		addr_wr_27 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 27) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_27 <= din;
		cmp_din_27 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_28 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 28) ? we : 0;
		addr_wr_28 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 28) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_28 <= din;
		cmp_din_28 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_29 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 29) ? we : 0;
		addr_wr_29 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 29) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_29 <= din;
		cmp_din_29 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_30 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 30) ? we : 0;
		addr_wr_30 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 30) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_30 <= din;
		cmp_din_30 <= cmp_din;
	end
...
always @ (posedge clk) 
	if (rst) begin
		...
		we_31 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 31) ? we : 0;
		addr_wr_31 <= (addr_wr[TCAM_ADDR_WIDTH+ID_BITS-1:TCAM_ADDR_WIDTH] == 31) ? addr_wr[TCAM_ADDR_WIDTH-1:0] : 0;
		din_31 <= din;
		cmp_din_31 <= cmp_din;
	end
